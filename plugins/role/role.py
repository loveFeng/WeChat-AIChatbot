...
class Role(Plugin):
    ...
    def load_data(self):
        config_path = os.path.join(self.file_dir, 'datas.json')
        try:
            with open(config_path, 'r') as f:
                self.role_info = json.load(f)
                logger.info("[Role]datas loaded {}".format(self.role_info))
                bot = Bridge().get_bot("chat")
                for sessionid, role in self.role_info.items():
                    self.roleplays[sessionid] = RolePlay(bot, sessionid, self.roles[role]["descn"],
                                                         self.roles[role].get("wrapper", "%s"))
        except Exception as e:
            logger.info("[Role]datas error: {}".format(e))
            self.role_info = {}
    ...
    def save_datas(self):
        ...
        try:
            with open(config_path, 'w') as f:
                json.dump(self.role_info, f)
                logger.info("[Role]datas saved.")
        except Exception as e:
            logger.info("[Role]datas error: {}".format(e))
