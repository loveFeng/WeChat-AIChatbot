...
class timetask(Plugin):
    ...
    def add_timeTask(self, content, e_context: EventContext):
        ...
        if taskModel.isPerson_makeGrop():
            newEvent, groupTitles = taskModel.get_Persion_makeGropTitle_eventStr()
    ...
    def replay_use_custom(self, model: TimeTaskModel, reply_text: str, replyType: ReplyType, context :Context, retry_cnt=0):
        ...
        try:
            ...
            if model.isPerson_makeGrop():
                _, groupTitles = model.get_Persion_makeGropTitle_eventStr()
                for groupTitle in groupTitles.split(","):
                    context["receiver"] = groupTitle
    ...
    def runTimeTask(self, model: TimeTaskModel):
        ...
        # 是否个人为群聊制定的任务
        if model.isPerson_makeGrop():
            newEvent, groupTitles = model.get_Persion_makeGropTitle_eventStr()
            groupTitle = groupTitles.split(",")[0]
            eventStr = newEvent
            channel_name = RobotConfig.conf().get("channel_type", "wx")
